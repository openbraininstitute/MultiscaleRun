# ruff: noqa: F841
import numpy as np
from constants import ATDMP, ETC, Creatine, Ephys, GeneralParameters
...
def compute_du(u, p, t):
    ...
    ## aliases
    u_h_c_n = GeneralParameters.C_H_cyt_n
    u_h_c_a = GeneralParameters.C_H_cyt_a
    u_co2_m_n = GeneralParameters.CO2_mito_n
    u_co2_m_a = GeneralParameters.CO2_mito_a
    u_ppi_c_a = GeneralParameters.PPi_a0
    u_notBigg_PHKa_c_a = GeneralParameters.PHKa_a0
    u_notBigg_PP1_c_a_initialValue = GeneralParameters.PP1_a0
    ...
    f_notBigg_J_Hle_n = ETC.x_Hle*u_notBigg_MitoMembrPotent_m_n*((1e-3*u_h_i_n)*exp(ETC.etcF*u_notBigg_MitoMembrPotent_m_n/ETC.etcRT)-(1e-3*u_h_m_n))/(np.exp(ETC.etcF*u_notBigg_MitoMembrPotent_m_n/ETC.etcRT)-1)
    ...
    du[5] = 0.5*T2Jcorrection*(1000*(+f_NADH2_u10mi_n-f_CYOR_u10mi_n)/W_x)
    du[6] = 0.5*T2Jcorrection*(1000*(+2*f_CYOR_u10mi_n-2*f_CYOOm2i_n)/W_i)
    du[7] = notBigg_JO2fromCap2n(u_o2_b_b,u_o2_c_n)-0.6*notBigg_vMitooutn_n(u_nadh_m_n,u_nad_m_n,u_o2_c_n,u_adp_i_n,u_atp_i_n)
    du[8] = 0.5*T2Jcorrection*(1000*(+f_ATPS4mi_n-f_ATPtm_n)/W_x)
    du[9] = 0.5*T2Jcorrection*(1000*(-f_ATPS4mi_n+f_ATPtm_n)/W_x)
    ...
    du[12] = 0.5*T2Jcorrection*(1000*(-f_ATPS4mi_n+f_notBigg_J_Pi1_n)/W_x)
    du[13] = 0.5*T2Jcorrection*(1000*(+notBigg_J_ATP_n(u_atp_c_n,u_atp_i_n)+f_ATPtm_n+ADK1m_n(u_amp_i_n,u_atp_i_n,u_adp_i_n))/W_i)
    du[14] = 0.5*T2Jcorrection*(1000*(+notBigg_J_ADP_n(u_adp_i_n,u_adp_c_n)-f_ATPtm_n-2*ADK1m_n(u_amp_i_n,u_atp_i_n,u_adp_i_n))/W_i)
    ...
    du[18] = 0.5*T2Jcorrection*(1000*(-f_notBigg_J_Pi1_n+notBigg_J_Pi2_n(u_pi_i_n,u_pi_c_n))/W_i)
    du[19] = 0.5*T2Jcorrection*(4*f_NADH2_u10mi_n+2*f_CYOR_u10mi_n+4*f_CYOOm2i_n-n_A*f_ATPS4mi_n-f_ATPtm_n-f_notBigg_J_Hle_n-notBigg_J_K_n(u_k_m_n,u_notBigg_MitoMembrPotent_m_n))/CIM
    du[20] = 0
    du[21] = 0
    du[22] = 0
    ...
    du[24] = 0
    du[25] = T2Jcorrection*(0.5*1000*f_r0509_n/W_x-FUMm_n(u_fum_m_n,u_mal_L_m_n))
    ...
    du[28] = T2Jcorrection*(0.5*SUCOASm_n(u_coa_m_n,u_succ_m_n,u_pi_m_n,u_adp_m_n,u_succoa_m_n,u_atp_m_n)-0.5*1000*f_r0509_n/W_x)+0.5*T2Jcorrection*OCOAT1m_n(u_acac_c_n,u_aacoa_m_n,u_succoa_m_n,u_succ_m_n)
...
if __name__ == "__main__":
    ...
    u = np.ones(UIdx.size) * 1.0  # all ones
