function metabolism!(du,u,p,t)
    ...
    """
           This function defines the metabolism model in a system of ordinary differential equations. It is called in the multiscale_run orchestrator.
           For more details on the metabolic model, refer to the original paper DOI: https://doi.org/10.1101/2023.08.30.555341
       ...
       variable idx to variable name: variable_idxc.jl
       ...
       mito_scale - mitochondiral scaling based on layer
       
       notBigg_FinDyn_W2017, notBigg_Fout_W2017, u_notBigg_vV_b_b - Blood Flow parameters 
   
   
       Returns:
   
           Formally, there is no "return" of this function, but the function itself is being used for metabolism dynamics simulation from multiscale orchestrator:
       ...
       NADtot_n = NAD_aging_coeff_mn*NADtot
       NADtot_a = NAD_aging_coeff_ma*NADtot
   ...
   r0509_n(u_nadh_m_n,u_pi_m_n) = 1*(mito_scale * x_DH*(r_DH*NAD_x_n-(1e-3*u_nadh_m_n))*((1+(1e-3*u_pi_m_n)/k_Pi1)/(1+(1e-3*u_pi_m_n)/k_Pi2)))
   NADH2_u10mi_n(u_nadh_m_n,u_q10h2_m_n) = 0.7759912793200542*(mito_scale *x_C1*(exp(-(dG_C1op_n+4*dG_H_n)/etcRT)*(1e-3*u_nadh_m_n)*Q_n-NAD_x_n*(1e-3*u_q10h2_m_n)))
   CYOR_u10mi_n(u_focytC_m_n,u_notBigg_MitoMembrPotent_m_n,u_q10h2_m_n,u_pi_m_n) = 0.6869109286423211*(mito_scale *x_C3*((1+(1e-3*u_pi_m_n)/k_Pi3)/(1+(1e-3*u_pi_m_n)/k_Pi4))*(exp(-(dG_C3op_n+4*dG_H_n-2*etcF*u_notBigg_MitoMembrPotent_m_n)/(2*etcRT))*Cox_n*(1e-3*u_q10h2_m_n)^0.5-(1e-3*u_focytC_m_n)*Q_n^0.5))
   CYOOm2i_n(u_focytC_m_n,u_notBigg_MitoMembrPotent_m_n,u_notBigg_Ctot_m_n,u_o2_c_n) = 0.6922236219048762*(mito_scale *x_C4*((1e-3*u_o2_c_n)/((1e-3*u_o2_c_n)+k_O2))*((1e-3*u_focytC_m_n)/(1e-3*u_notBigg_Ctot_m_n))*(exp(-(dG_C4op_n+2*dG_H_n)/(2*etcRT))*(1e-3*u_focytC_m_n)*((1e-3*u_o2_c_n)^0.25)-Cox_n*exp(etcF*u_notBigg_MitoMembrPotent_m_n/etcRT)))
   ATPS4mi_n(u_notBigg_ATP_mx_m_n,u_notBigg_ADP_mx_m_n,u_pi_m_n) = 0.6503896026734852*(mito_scale * x_F1*(exp(-(dG_F1op_n-n_A*dG_H_n)/etcRT)*(K_DD/K_DT)*(1e-3*u_notBigg_ADP_mx_m_n)*(1e-3*u_pi_m_n)-(1e-3*u_notBigg_ATP_mx_m_n)))
   ATPtm_n(u_notBigg_MitoMembrPotent_m_n) = 0.5562045860776595*mito_scale * (x_ANT*(ADP_fi_n/(ADP_fi_n+ATP_fi_n*exp(-etcF*(0.35*u_notBigg_MitoMembrPotent_m_n)/etcRT))-ADP_fx_n/(ADP_fx_n+ATP_fx_n*exp(-etcF*(-0.65*u_notBigg_MitoMembrPotent_m_n)/etcRT)))*(ADP_fi_n/(ADP_fi_n+k_mADP)))
   notBigg_J_Pi1_n(u_h_m_n,u_h_i_n) = 1*mito_scale * (x_Pi1*((1e-3*u_h_m_n)*H2PIi_n-(1e-3*u_h_i_n)*H2PIx_n)/(H2PIi_n+k_PiH))
   notBigg_J_Hle_n(u_h_m_n,u_h_i_n,u_notBigg_MitoMembrPotent_m_n) = 1*mito_scale * (x_Hle*u_notBigg_MitoMembrPotent_m_n*((1e-3*u_h_i_n)*exp(etcF*u_notBigg_MitoMembrPotent_m_n/etcRT)-(1e-3*u_h_m_n))/(exp(etcF*u_notBigg_MitoMembrPotent_m_n/etcRT)-1))
   notBigg_J_KH_n(u_h_m_n,u_h_i_n,u_k_m_n) = 1*mito_scale * (x_KH*(K_i*(1e-3*u_h_m_n)-(1e-3*u_k_m_n)*(1e-3*u_h_i_n)))
   notBigg_J_K_n(u_k_m_n,u_notBigg_MitoMembrPotent_m_n) = 1*mito_scale * (x_K*u_notBigg_MitoMembrPotent_m_n*(K_i*exp(etcF*u_notBigg_MitoMembrPotent_m_n/etcRT)-(1e-3*u_k_m_n))/(exp(etcF*u_notBigg_MitoMembrPotent_m_n/etcRT)-1))
   ADK1m_n(u_amp_i_n,u_atp_i_n,u_adp_i_n) = 0.8372574193451378*mito_scale * (x_AK*(K_AK*(1e-3*u_adp_i_n)*(1e-3*u_adp_i_n)-(1e-3*u_amp_i_n)*(1e-3*u_atp_i_n)))
   notBigg_J_AMP_n(u_amp_i_n,u_amp_c_n) = 1*mito_scale * (gamma*x_A*((1e-3*u_amp_c_n)-(1e-3*u_amp_i_n)))
   notBigg_J_ADP_n(u_adp_i_n,u_adp_c_n) = 1*mito_scale * (gamma*x_A*((1e-3*u_adp_c_n)-(1e-3*u_adp_i_n)))
   notBigg_J_ATP_n(u_atp_c_n,u_atp_i_n) = 1*mito_scale * (gamma*x_A*((1e-3*u_atp_c_n)-(1e-3*u_atp_i_n)))
   notBigg_J_Pi2_n(u_pi_i_n,u_pi_c_n) = 1*mito_scale * (gamma*x_Pi2*(1e-3*u_pi_c_n-(1e-3*u_pi_i_n)))
   notBigg_J_Ht_n(u_h_i_n,u_h_c_n) = 1*mito_scale * (gamma*x_Ht*(1e-3*u_h_c_n-(1e-3*u_h_i_n)))
   notBigg_J_MgATPx_n(u_notBigg_ATP_mx_m_n,u_mg2_m_n) = 1*mito_scale * (x_MgA*(ATP_fx_n*(1e-3*u_mg2_m_n)-K_DT*(1e-3*u_notBigg_ATP_mx_m_n)))
   notBigg_J_MgADPx_n(u_mg2_m_n,u_notBigg_ADP_mx_m_n) = 1*mito_scale * (x_MgA*(ADP_fx_n*(1e-3*u_mg2_m_n)-K_DD*(1e-3*u_notBigg_ADP_mx_m_n)))
   notBigg_J_MgATPi_n(u_notBigg_ATP_mi_i_n) = 1*mito_scale * (x_MgA*(ATP_fi_n*Mg_i_n-K_DT*(1e-3*u_notBigg_ATP_mi_i_n)))
   notBigg_J_MgADPi_n(u_notBigg_ADP_mi_i_n) = 1*mito_scale * (x_MgA*(ADP_fi_n*Mg_i_n-K_DD*(1e-3*u_notBigg_ADP_mi_i_n)))
   PDHm_n(u_pyr_m_n,u_coa_m_n,u_nad_m_n) = 0.6028430831681391*(mito_scale *VmaxPDHCmito_n*(u_pyr_m_n/(u_pyr_m_n+KmPyrMitoPDH_n))*(u_nad_m_n/(u_nad_m_n+KmNADmitoPDH_na))*(u_coa_m_n/(u_coa_m_n+KmCoAmitoPDH_n)))
   CSm_n(u_accoa_m_n,u_oaa_m_n,u_cit_m_n,u_coa_m_n) = 0.6877116657855111*(mito_scale * VmaxCSmito_n*(u_oaa_m_n/(u_oaa_m_n+KmOxaMito_n*(1.0+u_cit_m_n/KiCitMito_n)))*(u_accoa_m_n/(u_accoa_m_n+KmAcCoAmito_n*(1.0+u_coa_m_n/KiCoA_n))))
   ACONTm_n(u_icit_m_n,u_cit_m_n) = 0.49879329872120226*(mito_scale * VmaxAco_n*(u_cit_m_n-u_icit_m_n/KeqAco_na)/(1.0+u_cit_m_n/KmCitAco_n+u_icit_m_n/KmIsoCitAco_n))
   ...
   ASPGLUm_n(u_h_c_n,u_glu_L_c_n,u_asp_L_m_n,u_asp_L_c_n,u_notBigg_MitoMembrPotent_m_n,u_glu_L_m_n,u_h_m_n) = 0.4503437195810003*mito_scale * (Vmaxagc_n*(u_asp_L_m_n*u_glu_L_c_n-u_asp_L_c_n*u_glu_L_m_n/((exp(u_notBigg_MitoMembrPotent_m_n)^(F/(R*T)))*(u_h_c_n/u_h_m_n)))/((u_asp_L_m_n+Km_aspmito_agc_n)*(u_glu_L_c_n+Km_glu_agc_n)+(u_asp_L_c_n+Km_asp_agc_n)*(u_glu_L_m_n+Km_glumito_agc_n)))
   AKGMALtm_n(u_mal_L_c_n,u_akg_m_n,u_akg_c_n,u_mal_L_m_n) = 0.5282482511021107*mito_scale * (Vmaxmakgc_n*(u_mal_L_c_n*u_akg_m_n-u_mal_L_m_n*u_akg_c_n)/((u_mal_L_c_n+Km_mal_mkgc_n)*(u_akg_m_n+Km_akgmito_mkgc_n)+(u_mal_L_m_n+Km_malmito_mkgc_n)*(u_akg_c_n+Km_akg_mkgc_n)))
   r0509_a(u_nadh_m_a,u_pi_m_a) = 1*mito_scale * (x_DH_a*(r_DH_a*NAD_x_a-(1e-3*u_nadh_m_a))*((1+(1e-3*u_pi_m_a)/k_Pi1)/(1+(1e-3*u_pi_m_a)/k_Pi2)))
   NADH2_u10mi_a(u_nadh_m_a,u_q10h2_m_a) = 0.8604610299528399*mito_scale * (x_C1*(exp(-(dG_C1op_a+4*dG_H_a)/etcRT)*(1e-3*u_nadh_m_a)*Q_a-NAD_x_a*(1e-3*u_q10h2_m_a)))
   CYOR_u10mi_a(u_focytC_m_a,u_q10h2_m_a,u_pi_m_a,u_notBigg_MitoMembrPotent_m_a) = 0.7720733909302896*mito_scale * (x_C3*((1+(1e-3*u_pi_m_a)/k_Pi3)/(1+(1e-3*u_pi_m_a)/k_Pi4))*(exp(-(dG_C3op_a+4*dG_H_a-2*etcF*u_notBigg_MitoMembrPotent_m_a)/(2*etcRT))*Cox_a*(1e-3*u_q10h2_m_a)^0.5-(1e-3*u_focytC_m_a)*Q_a^0.5))
   CYOOm2i_a(u_notBigg_Ctot_m_a,u_focytC_m_a,u_notBigg_MitoMembrPotent_m_a,u_o2_c_a) = 0.8836779655353504*mito_scale * (x_C4*((1e-3*u_o2_c_a)/((1e-3*u_o2_c_a)+k_O2))*((1e-3*u_focytC_m_a)/(1e-3*u_notBigg_Ctot_m_a))*(exp(-(dG_C4op_a+2*dG_H_a)/(2*etcRT))*(1e-3*u_focytC_m_a)*((1e-3*u_o2_c_a)^0.25)-Cox_a*exp(etcF*u_notBigg_MitoMembrPotent_m_a/etcRT)))
   ATPS4mi_a(u_notBigg_ATP_mx_m_a,u_pi_m_a,u_notBigg_ADP_mx_m_a) = 0.794495760350036*mito_scale * (x_F1_a*(exp(-(dG_F1op_a-n_A*dG_H_a)/etcRT)*(K_DD_a/K_DT_a)*(1e-3*u_notBigg_ADP_mx_m_a)*(1e-3*u_pi_m_a)-(1e-3*u_notBigg_ATP_mx_m_a)))
   ATPtm_a(u_notBigg_MitoMembrPotent_m_a) = 1*mito_scale * (x_ANT_a*(ADP_fi_a/(ADP_fi_a+ATP_fi_a*exp(-etcF*(0.35*u_notBigg_MitoMembrPotent_m_a)/etcRT))-ADP_fx_a/(ADP_fx_a+ATP_fx_a*exp(-etcF*(-0.65*u_notBigg_MitoMembrPotent_m_a)/etcRT)))*(ADP_fi_a/(ADP_fi_a+k_mADP_a)))
   notBigg_J_Pi1_a(u_h_i_a,u_h_m_a) = 1*mito_scale * (x_Pi1*((1e-3*u_h_m_a)*H2PIi_a-(1e-3*u_h_i_a)*H2PIx_a)/(H2PIi_a+k_PiH))
   notBigg_J_Hle_a(u_h_i_a,u_h_m_a,u_notBigg_MitoMembrPotent_m_a) = 1*mito_scale * (x_Hle*u_notBigg_MitoMembrPotent_m_a*((1e-3*u_h_i_a)*exp(etcF*u_notBigg_MitoMembrPotent_m_a/etcRT)-(1e-3*u_h_m_a))/(exp(etcF*u_notBigg_MitoMembrPotent_m_a/etcRT)-1))
   notBigg_J_KH_a(u_h_i_a,u_h_m_a,u_k_m_a) = 1*mito_scale * (x_KH*(K_i*(1e-3*u_h_m_a)-(1e-3*u_k_m_a)*(1e-3*u_h_i_a)))
   notBigg_J_K_a(u_k_m_a,u_notBigg_MitoMembrPotent_m_a) = 1*mito_scale * (x_K*u_notBigg_MitoMembrPotent_m_a*(K_i*exp(etcF*u_notBigg_MitoMembrPotent_m_a/etcRT)-(1e-3*u_k_m_a))/(exp(etcF*u_notBigg_MitoMembrPotent_m_a/etcRT)-1))
   ADK1m_a(u_atp_i_a,u_adp_i_a,u_amp_i_a) = 1*mito_scale * (x_AK*(K_AK*(1e-3*u_adp_i_a)*(1e-3*u_adp_i_a)-(1e-3*u_amp_i_a)*(1e-3*u_atp_i_a)))
   notBigg_J_AMP_a(u_amp_i_a,u_amp_c_a) = 1*mito_scale * (gamma*x_A*((1e-3*u_amp_c_a)-(1e-3*u_amp_i_a)))
   notBigg_J_ADP_a(u_adp_c_a,u_adp_i_a) = 1*mito_scale * (gamma*x_A*((1e-3*u_adp_c_a)-(1e-3*u_adp_i_a)))
   notBigg_J_ATP_a(u_atp_i_a,u_atp_c_a) = 1*mito_scale * (gamma*x_A*((1e-3*u_atp_c_a)-(1e-3*u_atp_i_a)))
   notBigg_J_Pi2_a(u_pi_i_a,u_pi_c_a) = 1*mito_scale * (gamma*x_Pi2*(1e-3*u_pi_c_a-(1e-3*u_pi_i_a)))
   notBigg_J_Ht_a(u_h_i_a,u_h_c_a) = 1*mito_scale * (gamma*x_Ht*(1e-3*u_h_c_a-(1e-3*u_h_i_a)))
   notBigg_J_MgATPx_a(u_notBigg_ATP_mx_m_a,u_mg2_m_a) = 1*mito_scale * (x_MgA*(ATP_fx_a*(1e-3*u_mg2_m_a)-K_DT_a*(1e-3*u_notBigg_ATP_mx_m_a)))
   notBigg_J_MgADPx_a(u_notBigg_ADP_mx_m_a,u_mg2_m_a) = 1*mito_scale * (x_MgA*(ADP_fx_a*(1e-3*u_mg2_m_a)-K_DD_a*(1e-3*u_notBigg_ADP_mx_m_a)))
   notBigg_J_MgATPi_a(u_notBigg_ATP_mi_i_a) = 1*mito_scale * (x_MgA*(ATP_fi_a*Mg_i_a-K_DT_a*(1e-3*u_notBigg_ATP_mi_i_a)))
   notBigg_J_MgADPi_a(u_notBigg_ADP_mi_i_a) = 1*mito_scale * (x_MgA*(ADP_fi_a*Mg_i_a-K_DD_a*(1e-3*u_notBigg_ADP_mi_i_a)))
   PDHm_a(u_nad_m_a,u_pyr_m_a,u_coa_m_a) = 0.6203100331393553*mito_scale * (VmaxPDHCmito_a*(u_pyr_m_a/(u_pyr_m_a+KmPyrMitoPDH_a))*(u_nad_m_a/(u_nad_m_a+KmNADmitoPDH_na))*(u_coa_m_a/(u_coa_m_a+KmCoAmitoPDH_a)))
   CSm_a(u_oaa_m_a,u_cit_m_a,u_accoa_m_a,u_coa_m_a) = 1*mito_scale * (VmaxCSmito_a*(u_oaa_m_a/(u_oaa_m_a+KmOxaMito_a*(1.0+u_cit_m_a/KiCitMito_a)))*(u_accoa_m_a/(u_accoa_m_a+KmAcCoAmito_a*(1.0+u_coa_m_a/KiCoA_a))))
   ACONTm_a(u_cit_m_a,u_icit_m_a) = 0.48342767640154766*mito_scale * (VmaxAco_a*(u_cit_m_a-u_icit_m_a/KeqAco_na)/(1.0+u_cit_m_a/KmCitAco_a+u_icit_m_a/KmIsoCitAco_a))
   ...
   SUCOASm_a(u_coa_m_a,u_succoa_m_a,u_atp_m_a,u_succ_m_a,u_pi_m_a,u_adp_m_a) = 1.3146510580492397*mito_scale * (VmaxSuccoaATPscs_a*(1+AmaxPscs_a*((u_pi_m_a^npscs_a)/((u_pi_m_a^npscs_a)+(Km_pi_scs_na^npscs_a))))*(u_succoa_m_a*u_adp_m_a*u_pi_m_a-u_succ_m_a*u_coa_m_a*u_atp_m_a/Keqsuccoascs_na)/((1+u_succoa_m_a/Km_succoa_scs_a)*(1+u_adp_m_a/Km_ADPmito_scs_a)*(1+u_pi_m_a/Km_pi_scs_na)+(1+u_succ_m_a/Km_succ_scs_a)*(1+u_coa_m_a/Km_coa_scs_a)*(1+u_atp_m_a/Km_atpmito_scs_a)))
   FUMm_a(u_mal_L_m_a,u_fum_m_a) = 0.6995126037645376*mito_scale * (Vmaxfum_a*(u_fum_m_a-u_mal_L_m_a/Keqfummito_na)/(1.0+u_fum_m_a/Km_fummito_a+u_mal_L_m_a/Km_malmito_a))
   MDHm_a(u_nad_m_a,u_oaa_m_a,u_mal_L_m_a,u_nadh_m_a) = 1.2266010315633475*mito_scale * (VmaxMDHmito_a*(u_mal_L_m_a*u_nad_m_a-u_oaa_m_a*u_nadh_m_a/Keqmdhmito_na)/((1.0+u_mal_L_m_a/Km_mal_mdh_a)*(1.0+u_nad_m_a/Km_nad_mdh_na)+(1.0+u_oaa_m_a/Km_oxa_mdh_a)*(1.0+u_nadh_m_a/Km_nadh_mdh_na)))
   PCm_a(u_co2_m_a,u_oaa_m_a,u_pyr_m_a,u_atp_m_a,u_adp_m_a) = 1*mito_scale * (((u_atp_m_a/u_adp_m_a)/(muPYRCARB_a+(u_atp_m_a/u_adp_m_a)))*VmPYRCARB_a*(u_pyr_m_a*u_co2_m_a-u_oaa_m_a/KeqPYRCARB_a)/(KmPYR_PYRCARB_a*KmCO2_PYRCARB_a+KmPYR_PYRCARB_a*u_co2_m_a+KmCO2_PYRCARB_a*u_pyr_m_a+u_co2_m_a*u_pyr_m_a))
   ...
   GLUNm_n(u_gln_L_c_n,u_glu_L_m_n) = 0.8547630950925981*mito_scale * (mito_scale * VmGLS_n*(u_gln_L_c_n-u_glu_L_m_n/KeqGLS_n)/(KmGLNGLS_n*(1.0+u_glu_L_m_n/KiGLUGLS_n)+u_gln_L_c_n))
   ...
   notBigg_jLacDiff_e(u_lac_L_e_e) = 0
   ...
   PYRt2m_n(u_h_m_n,u_pyr_m_n,u_h_c_n,u_pyr_c_n) = 1.1810410492580787*(mito_scale * Vmax_PYRtrcyt2mito_nH*(u_pyr_c_n*u_h_c_n-u_pyr_m_n*u_h_m_n)/((1.0+u_pyr_c_n/KmPyrCytTr_n)*(1.0+u_pyr_m_n/KmPyrMitoTr_n)))
   PYRt2m_a(u_h_m_a,u_pyr_c_a,u_h_c_a,u_pyr_m_a) = 1.1210261302822038*(mito_scale * (Vmax_PYRtrcyt2mito_aH*(u_pyr_c_a*u_h_c_a-u_pyr_m_a*u_h_m_a)/((1.0+u_pyr_c_a/KmPyrCytTr_a)*(1.0+u_pyr_m_a/KmPyrMitoTr_a))))
   notBigg_vShuttlen(u_nadh_m_n,u_nadh_c_n) = NADHshuttle_aging_n*mito_scale *(TnNADH_jlv*(u_nadh_c_n/(NAD_aging_coeff_n*0.212-u_nadh_c_n))/(MnCyto_jlv+(u_nadh_c_n/(NAD_aging_coeff_n*0.212-u_nadh_c_n)))*((1000*NADtot_n-u_nadh_m_n)/u_nadh_m_n)/(MnMito_jlv+((1000*NADtot_n-u_nadh_m_n)/u_nadh_m_n)))
   notBigg_vShuttleg(u_nadh_c_a,u_nadh_m_a) = NADHshuttle_aging_a*mito_scale* (TgNADH_jlv*(u_nadh_c_a/(NAD_aging_coeff_a*0.212-u_nadh_c_a))/(MgCyto_jlv+(u_nadh_c_a/(NAD_aging_coeff_a*0.212-u_nadh_c_a)))*((1000*NADtot_a-u_nadh_m_a)/u_nadh_m_a)/(MgMito_jlv+((1000*NADtot_a-u_nadh_m_a)/u_nadh_m_a)))
   notBigg_vMitooutn_n(u_nadh_m_n,u_nad_m_n,u_o2_c_n,u_adp_i_n,u_atp_i_n) = 1*mito_scale * (V_oxphos_n*((1/(u_atp_i_n/u_adp_i_n))/(mu_oxphos_n+(1/(u_atp_i_n/u_adp_i_n))))*((u_nadh_m_n/u_nad_m_n)/(nu_oxphos_n+(u_nadh_m_n/u_nad_m_n)))*(u_o2_c_n/(u_o2_c_n+K_oxphos_n)))
   notBigg_vMitooutg_a(u_atp_i_a,u_nadh_m_a,u_nad_m_a,u_o2_c_a,u_adp_i_a) = 1*mito_scale * (V_oxphos_a*((1/(u_atp_i_a/u_adp_i_a))/(mu_oxphos_a+(1/(u_atp_i_a/u_adp_i_a))))*((u_nadh_m_a/u_nad_m_a)/(nu_oxphos_a+(u_nadh_m_a/u_nad_m_a)))*(u_o2_c_a/(u_o2_c_a+K_oxphos_a)))
   notBigg_vMitoinn(u_pyr_m_n,u_nadh_m_n) = 1*mito_scale *(VMaxMitoinn*u_pyr_m_n/(u_pyr_m_n+KmMito)*(1000*NADtot_n-u_nadh_m_n)/(1000*NADtot_n-u_nadh_m_n+KmNADn_jlv))
   notBigg_vMitoing(u_nadh_m_a,u_pyr_m_a) = 1*mito_scale*(VMaxMitoing*u_pyr_m_a/(u_pyr_m_a+KmMito_a)*(1000*NADtot_a-u_nadh_m_a)/(1000*NADtot_a-u_nadh_m_a+KmNADg_jlv))
