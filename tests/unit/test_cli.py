...
def test_simple_init(tmp_path):
    """Test that a standard init is a dual-run"""
    sim_path = tmp_path / "sim"
    main(args=["init", str(sim_path)])
    ...
    try:
        assert simulation_config["multiscale_run"]["with_metabolism"]
        assert not simulation_config["multiscale_run"]["with_bloodflow"]
        assert not simulation_config["multiscale_run"]["with_steps"]
...
def test_init_twice(tmp_path):
    sim_path = tmp_path / "sim"
    main(args=["init", str(sim_path)])
    # already created. Complain
    with pytest.raises(utils.MsrException):
        main(args=["init", str(sim_path)])
    # -f should not complain
    main(args=["init", "-f", str(sim_path)])
